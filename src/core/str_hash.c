...
void string_hash_init(StringHashTable *table, const int init_size) {
  table->cap = init_size;
  table->free_head = 0;
  table->heads = malloc(init_size * sizeof(int));
  table->entries = malloc(init_size * sizeof(StringHashEntry));

  memset(table->heads, -1, init_size * sizeof(int));
  for (int i = 0; i < init_size; i++) table->entries[i].next = i + 1;
}
...
void string_hash_insert(const StringHashTable *table, const char *str,
  ...
  int *head = table->heads + entry->hash % table->cap;
  entry->next = *head;
  *head = id;
}
...
int string_hash_remove(StringHashTable *table, const char *str) {
  ...
  int *ptr = table->heads + hash % table->cap;
  for (int i; (i = *ptr) != -1;) {
...
int string_hash_find(const StringHashTable *table, const char *str) {
  const uint32_t hash = str_hash(str);
  for (int i = table->heads[hash % table->cap]; i != -1;) {
...
void string_hash_resize(StringHashTable *table, const int new_size) {
  ...
  for (int i = old_size; i < new_size; i++) table->entries[i].next = i + 1;
  ...
  for (int i = 0; i < old_size; i++) {
    StringHashEntry *entry = table->entries + i;
    int *new_head = table->heads + entry->hash % new_size;
    entry->next = *new_head;
    *new_head = i;
  }
}