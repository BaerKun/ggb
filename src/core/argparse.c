...
static void argparse_error(struct argparse *self,
  ...
  (void)self;
  if (flags & OPT_LONG) {
    push_error_fmt("option `--%s` %s\n", opt->long_name, reason);
  } else {
    push_error_fmt("option `-%c` %s\n", opt->short_name, reason);
  }
  parse_fail = 1;
}
...
int argparse_parse(struct argparse *self, int argc, const char **argv) {
  ...
  unknown:
    push_error_fmt("unknown option `%s`\n", self->argv[0]);
    // argparse_usage(self);
  ...
  const int remaining = self->cpidx + self->argc;
  return parse_fail ? ~remaining : remaining;
}
